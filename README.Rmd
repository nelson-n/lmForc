...
Additional helper functions for working with lmForc `Forecast` objects are available in the directory */lmForc_helpers*. These extension functions include `subset_identical()` which subsets a list of forecasts to an identical sample period and `transform_bytime()` which converts forecasts to *Time Format*. These functions can be accessed by cloning the scripts *lmForc_subset.R*, *lmForc_transform.R*, and *lmForc_visualize.R* from the */lmForc_helpers* directory and running `source("lmForc_subset.R")`, `source("lmForc_transform.R")`, and `source("lmForc_visualize.R")` in R.
