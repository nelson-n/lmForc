...
train_lm = lm(y ~ x1 + x2, lm_call$model[1:5, ])

pos5 <- train_lm$coefficients[[1]] +
  train_lm$coefficients[[2]] * data$x1[5 + h_ahead] +
  train_lm$coefficients[[3]] * data$x2[5 + h_ahead]

# Test forecast for period 7.
train_lm = lm(y ~ x1 + x2, lm_call$model[1:7, ])

pos7 <- train_lm$coefficients[[1]] +
